...
describe('Add function Tests', () => {
    ...
    it("should return an error when no kid found", async () => {
        ...
        expect(result.body).not.toBeNull()
        expect(JSON.parse(result.body).message).toStrictEqual( `Item with id ${kidId} not found`);
    });

    it("should return an error when no path param id is send in the request", async () => {
        const getResponse = { id: kidId,  name: 'name-kid1', surname: 'surname-kid1'} 
        ...
        expect(result.body).not.toBeNull()
        expect(JSON.parse(result.body).message).toBe('Error in the path params, `id` is expected')
    });
});
