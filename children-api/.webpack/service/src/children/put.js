...
exports.kidSchema = void 0;
const zod_1 = __webpack_require__(/*! zod */ "zod");
exports.kidSchema = zod_1.z.object({
    KidName: zod_1.z
        .string({
...
const kid_1 = __webpack_require__(/*! ./kid */ "./src/children/kid.ts");
const repository_1 = __webpack_require__(/*! @libs/repository */ "./src/libs/repository.ts");
const put = async (event) => {
    if (event.body) {
        try {
            const item = kid_1.kidSchema.parse(JSON.parse(event.body));
            await (0, repository_1.saveChildren)(item);
            console.info('Success - item added or updated');
            return (0, util_1.formatJSONResponse)({ kid: item }, 201);
        }
        catch (err) {
            if (err instanceof zod_1.ZodError) {
                return (0, util_1.formatJSONResponse)({ message: 'Error in the request body' }, 400);
            }
            return (0, util_1.formatJSONResponse)({ message: err.message }, 500);
        }
    }
    else {
        return (0, util_1.formatJSONResponse)({ message: 'The body cannot be empty' }, 400);
    }
};
const handler = async (event) => put(event);
