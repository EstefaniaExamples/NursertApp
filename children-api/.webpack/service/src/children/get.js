...
const async_method_1 = __webpack_require__(/*! ./async-method */ "./src/children/async-method.ts");
const repository_1 = __webpack_require__(/*! @libs/repository */ "./src/libs/repository.ts");
const asyncMsg = (async () => {
    return await (0, async_method_1.main)();
})();
const getChildren = async () => {
    console.info('INFO: Starting get all kids handler');
    console.info('withing get method ' + await asyncMsg);
    try {
        const items = await (0, repository_1.getAllChildren)();
        return (0, util_1.formatJSONResponse)({ kids: items });
    }
    catch (err) {
        console.error(err);
...
const handler = async () => getChildren();
