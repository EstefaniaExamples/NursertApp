...
const getItemById = async (event) => {
    ...
    if (event.pathParameters && event.pathParameters.id) {
        try {
            return (0, util_1.formatJSONResponse)({
                kid: await (0, repository_1.getChildrenById)(event.pathParameters?.['id'] || '')
            });
        }
        catch (err) {
            console.error(err);
...
const handler = async (event) => getItemById(event);
