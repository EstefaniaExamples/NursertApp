...
const get = async () => {
    ...
    return await ddbClient
        ...
        .then(data => data.Items?.map(element => `${element.KidId.S} => ${element.KidName.S} ${element.KidSurname.S} (${element.BirthDate.S})`) || [])
        .then(kidsList => simpleHttpResponse({ children: kidsList }));
};
const wrapper = async () => get().catch((err) => simpleHttpResponse({ message: err.message }, 500));
