...
const get = async (event) => {
    ...
    if (event.pathParameters && event.pathParameters.id) {
        ...
        return simpleHttpResponse({ message: 'Error in the path params, `id` is expected' }, 400);
    }
};
const wrapper = async (event) => get(event).catch((err) => simpleHttpResponse({ message: err.message }, 500));
